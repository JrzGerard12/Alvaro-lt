...
def Moneris(card):
    try:
        UseMail = BehaviorsBraintree().RandomName('correo')
        session = Session()
        ...
        if '{"error"' in response.text: 
            ...
            if '"DECLINED           * REFER CALL TO ISSUE=' in response.text:return 'Approved! ✅',response.json()['error']
            elif 'Invalid Card CVV' in response.text: return 'Approved! ✅',response.json()['error']
            elif 'Name, Address or CVV code is incorrect.' in response.text: return 'Approved! ✅',response.json()['error']
            return 'Declined! ❌',response.json()['error']
        
        else: return 'Approved! ✅','Charged $80.00'
